package com.Cbic_Aakalan_Project.dao.Query.CGST;
...
//                "GROUP BY zc.ZONE_NAME, cc.COMM_NAME, zc.ZONE_CODE;\n";
    ...
    public String QueryFor_Adjudication_Legacy_6_ParticularZoneWise(String month_date, String zone_code){
        ...
        String query_assessment = "WITH cte1 AS (\n" +
                "    SELECT zc.ZONE_NAME, cc.ZONE_CODE, cc.COMM_NAME,(14a.COMM_DISPOSAL_NO + 14a.JC_DISPOSAL_NO + 14a.AC_DISPOSAL_NO + 14a.SUP_DISPOSAL_NO) AS col9_6a,\n" +
                "        (SELECT \n" +
                "            (14b.COMM_CLOSING_NO + 14b.JC_CLOSING_NO + 14b.AC_CLOSING_NO + 14b.SUP_CLOSING_NO)\n" +
                "         FROM mis_dgi_st_1a 14b \n" +
                "         WHERE 14b.MM_YYYY = '" + prev_month_new + "' AND 14b.COMM_CODE = cc.COMM_CODE) AS col3_6a\n" +
                ...
                "    WHERE 14a.MM_YYYY = '" + month_date + "' AND zc.ZONE_NAME NOT IN ('DG East', 'CEI DG')\n" +
                "),\n" +
                "cte3 AS (\n" +
                "    SELECT zc.ZONE_NAME, cc.ZONE_CODE, cc.COMM_NAME,\n" +
                "        CASE \n" +
                "            WHEN (14a.COMM_CLOSING_NO + 14a.JC_CLOSING_NO + 14a.AC_CLOSING_NO + 14a.SUP_CLOSING_NO) = 0 \n" +
                "            THEN 0\n" +
                ...
                "         FROM mis_dgi_ce_1a 14c \n" +
                "         WHERE 14c.MM_YYYY = '" + prev_month_new + "' AND 14c.COMM_CODE = cc.COMM_CODE) AS col3_6c\n" +
                "    FROM mis_gst_commcode cc\n" +
                "    RIGHT JOIN mis_dgi_ce_1a 14b ON cc.COMM_CODE = 14b.COMM_CODE\n" +
                "    LEFT JOIN mis_gst_zonecode zc ON zc.ZONE_CODE = cc.ZONE_CODE\n" +
                "    WHERE 14b.MM_YYYY = '" + month_date + "'\n" +
                "),\n" +
                "cte_median_6a AS (\n" +
                ...
                "    WHERE 14b.MM_YYYY = '" + month_date + "'\n" +
                ")\n" +
                "SELECT cte1.ZONE_NAME, cte1.ZONE_CODE, cte1.COMM_NAME,cte1.col9_6a, cte1.col3_6a,\n" +
                "    CASE WHEN cte1.col3_6a = 0 THEN 0 ELSE (cte1.col9_6a / cte1.col3_6a) * 100 END AS total_score_6a,\n" +
    ...
    public String QueryFor_Adjudication_Legacy_6_ParticularSubparameterWise(String month_date, String zone_code){
        ...
        String query_assessment = "WITH col9_data AS (\n" +
                "    SELECT zc.ZONE_NAME, cc.ZONE_CODE,\n" +
                "           SUM(14c.COMM_DISPOSAL_NO + 14c.JC_DISPOSAL_NO + 14c.AC_DISPOSAL_NO + 14c.SUP_DISPOSAL_NO) AS numerator\n" +
                "    FROM mis_gst_commcode AS cc\n" +
                "    RIGHT JOIN mis_dgi_st_1a AS 14c ON cc.COMM_CODE = 14c.COMM_CODE\n" +
                "    LEFT JOIN mis_gst_zonecode AS zc ON zc.ZONE_CODE = cc.ZONE_CODE\n" +
                "    WHERE 14c.MM_YYYY = '" + month_date + "'\n" +
                ...
                "           CASE\n" +
                "               WHEN col3_data.col3 = 0 THEN 0\n" +
                "               ELSE (col9_data.numerator / col3_data.col3) * 100\n" +
                "           END AS total_score\n" +
                "    FROM col9_data\n" +
                "    LEFT JOIN col3_data ON col9_data.ZONE_CODE = col3_data.ZONE_CODE AND col9_data.ZONE_NAME = col3_data.ZONE_NAME\n" +
                "),\n" +
                ...
                "    SELECT *, 0 AS numerator, 0 AS median\n" +
                "    FROM ranked_data_2\n" +
                "),\n" +
                "disposal_data AS (\n" +
                "    SELECT zc.ZONE_NAME, cc.ZONE_CODE,\n" +
                "           SUM(COALESCE(14c.COMM_DISPOSAL_NO, 0) + COALESCE(14c.JC_DISPOSAL_NO, 0) + COALESCE(14c.AC_DISPOSAL_NO, 0) + COALESCE(14c.SUP_DISPOSAL_NO, 0)) AS numerator,\n" +
                "           ROW_NUMBER() OVER (ORDER BY SUM(COALESCE(14c.COMM_DISPOSAL_NO, 0) + COALESCE(14c.JC_DISPOSAL_NO, 0) + COALESCE(14c.AC_DISPOSAL_NO, 0) + COALESCE(14c.SUP_DISPOSAL_NO, 0)) ) AS row_num,\n" +
                "           COUNT(*) OVER () AS total_count\n" +
                "    FROM mis_gst_commcode AS cc\n" +
                "    RIGHT JOIN mis_dgi_ce_1a AS 14c ON cc.COMM_CODE = 14c.COMM_CODE\n" +
                ...
                "           CASE\n" +
                "               WHEN total_count % 2 = 1 THEN\n" +
                "                   (SELECT numerator FROM disposal_data WHERE row_num = (total_count + 1) / 2)\n" +
                "               ELSE\n" +
                "                   (SELECT AVG(numerator) FROM disposal_data WHERE row_num IN (total_count / 2, total_count / 2 + 1))\n" +
                "           END AS median\n" +
                "    FROM disposal_data\n" +
                "    LIMIT 1\n" +
                "),\n" +
                "closing_data AS (\n" +
                "    SELECT zc.ZONE_NAME, cc.ZONE_CODE,\n" +
                "           SUM(COALESCE(14c.COMM_CLOSING_NO, 0) + COALESCE(14c.JC_CLOSING_NO, 0) + COALESCE(14c.AC_CLOSING_NO, 0) + COALESCE(14c.SUP_CLOSING_NO, 0)) AS col3\n" +
                "    FROM mis_gst_commcode AS cc\n" +
                "    RIGHT JOIN mis_dgi_ce_1a AS 14c ON cc.COMM_CODE = 14c.COMM_CODE\n" +
                "    LEFT JOIN mis_gst_zonecode AS zc ON zc.ZONE_CODE = cc.ZONE_CODE\n" +
                "    WHERE 14c.MM_YYYY = '" + month_date + "'\n" +
                ...
                "),\n" +
                "ranked_data_3 AS (\n" +
                "    SELECT d.ZONE_NAME, d.ZONE_CODE, d.numerator,\n" +
                "           COALESCE(c.col3, 0) AS col3,\n" +
                "           CASE WHEN COALESCE(c.col3, 0) = 0 THEN 0\n" +
                "           ELSE (d.numerator * 100 / COALESCE(c.col3, 0))\n" +
                "           END AS total_score\n" +
                "    FROM disposal_data AS d\n" +
                "    LEFT JOIN closing_data AS c ON d.ZONE_NAME = c.ZONE_NAME AND d.ZONE_CODE = c.ZONE_CODE\n" +
                ")\n" +
                "SELECT r.ZONE_NAME, r.ZONE_CODE, r.total_score, 'GST6A' AS gst, r.numerator, m.median, CONCAT(r.numerator, '/', col3) AS absvl\n" +
                "FROM ranked_data r\n" +
                "CROSS JOIN median_value m\n" +
                "WHERE r.ZONE_NAME NOT IN ('DG East', 'CEI DG') \n" +
                "  AND r.ZONE_CODE = '" + zone_code + "' \n" +
                "\n" +
                "UNION ALL\n" +
                "\n" +
                "SELECT ZONE_NAME, ZONE_CODE, total_score, 'GST6B' AS gst, numerator, median, CONCAT(col18, '/', col13) AS absvl\n" +
                "FROM ranked_data_with_rank\n" +
                "WHERE ZONE_CODE = '" + zone_code + "' \n" +
                "\n" +
                "UNION ALL\n" +
                "\n" +
                "SELECT t.ZONE_NAME, t.ZONE_CODE, t.total_score, 'GST6C' AS gst, t.numerator, m.median, CONCAT(t.numerator, '/', col3) AS absvl\n" +
                "FROM ranked_data_3 AS t\n" +
                ...
                "\n" +
                "SELECT zc.ZONE_NAME, cc.ZONE_CODE,\n" +
                "       COALESCE((SUM(14c.COMM_MORE_YEAR_AMT + 14c.JC_MORE_YEAR_AMT + 14c.AC_MORE_YEAR_AMT + 14c.SUP_MORE_YEAR_AMT) /\n" +
                "       NULLIF(SUM(14c.COMM_CLOSING_NO + 14c.JC_CLOSING_NO + 14c.AC_CLOSING_NO + 14c.SUP_CLOSING_NO), 0)) * 100, 0) AS total_score,\n" +
                "       'GST6D' AS gst,\n" +
                "       0 AS numerator, \n" +
                "       0 AS median,\n" +
                "       CONCAT(SUM(14c.COMM_MORE_YEAR_AMT + 14c.JC_MORE_YEAR_AMT + 14c.AC_MORE_YEAR_AMT + 14c.SUP_MORE_YEAR_AMT), \n" +
                "              '/', \n" +
                "              SUM(14c.COMM_CLOSING_NO + 14c.JC_CLOSING_NO + 14c.AC_CLOSING_NO + 14c.SUP_CLOSING_NO) ) AS absvl\n" +
                "FROM mis_gst_commcode AS cc\n" +
                "RIGHT JOIN mis_dgi_ce_1a AS 14c ON cc.COMM_CODE = 14c.COMM_CODE\n" +
                "LEFT JOIN mis_gst_zonecode AS zc ON zc.ZONE_CODE = cc.ZONE_CODE\n" +
                "WHERE 14c.MM_YYYY = '" + month_date + "'\n" +
    ...
    public String QueryFor_Adjudication_Legacy_6_AllCommissary(String month_date){
        ...
        String query_assessment = "WITH cte1 AS (\n" +
                ...
                "    FROM mis_gst_commcode cc \n" +
                "    RIGHT JOIN mis_dgi_st_1a 14a ON cc.COMM_CODE = 14a.COMM_CODE\n" +
                "    LEFT JOIN mis_gst_zonecode zc ON zc.ZONE_CODE = cc.ZONE_CODE WHERE 14a.MM_YYYY = '" + month_date + "'\n" +
                "),\n" +
                "cte4 AS (\n" +
                "    -- GST6C\n" +
                "    SELECT zc.ZONE_NAME, cc.COMM_NAME, cc.ZONE_CODE,(14b.COMM_DISPOSAL_NO + 14b.JC_DISPOSAL_NO + 14b.AC_DISPOSAL_NO + 14b.SUP_DISPOSAL_NO) AS col9_6c,\n" +
                "    (SELECT (14c.COMM_CLOSING_NO + 14c.JC_CLOSING_NO + 14c.AC_CLOSING_NO + 14c.SUP_CLOSING_NO) \n" +
                "     FROM mis_dgi_ce_1a 14c WHERE 14c.MM_YYYY = '" + prev_month_new + "' AND 14c.COMM_CODE = cc.COMM_CODE) AS col3_6c\n" +
                "FROM mis_gst_commcode cc\n" +
                "RIGHT JOIN mis_dgi_ce_1a 14b ON cc.COMM_CODE = 14b.COMM_CODE\n" +
                "LEFT JOIN mis_gst_zonecode zc ON zc.ZONE_CODE = cc.ZONE_CODE WHERE 14b.MM_YYYY = '" + month_date + "'\n" +
                "),\n" +
                "cte_median_6a AS (\n" +
    ...
    public String QueryFor_Appeals_11_ZoneWise(String month_date){
        String prev_month_new = DateCalculate.getPreviousMonth(month_date);
        String query_assessment = "WITH cte_11a AS (\n" +
                ...
                "    LEFT JOIN mis_gst_commcode AS cc ON 11a.COMM_CODE = cc.COMM_CODE \n" +
                "    LEFT JOIN mis_gst_zonecode AS zc ON cc.ZONE_CODE = zc.ZONE_CODE \n" +
                "    WHERE 11a.MM_YYYY = '" + prev_month_new + "' AND 11a.FORUM_CODE = 7 GROUP BY zc.ZONE_NAME, zc.ZONE_CODE\n" +
                "),\n" +
                "median_cte_11c AS (\n" +
                "    SELECT \n" +
    ...
    public String QueryFor_Appeals_11_ParticularZoneWise(String month_date, String zone_code){
        ...
        String query_assessment = "WITH \n"
                + "    -- CTE for Query 1 and Query 3 (FORUM_CODE = 6 and 7 for MM_YYYY = '" + month_date + "' )\n"
                + "    cte_11 AS (\n"
                + "        SELECT zc.ZONE_NAME, zc.ZONE_CODE, cc.COMM_NAME, cc.COMM_CODE,\n"
                + "               SUM(CASE WHEN 11a.FORUM_CODE = 6 THEN DEPARTMENT_DISPOSAL_NO + TAXPAYER_DISPOSAL_NO ELSE 0 END) AS numerator11a,\n"
                + "               SUM(CASE WHEN 11a.FORUM_CODE = 7 THEN DEPARTMENT_DISPOSAL_NO + TAXPAYER_DISPOSAL_NO ELSE 0 END) AS numerator11c\n"
                + "        FROM mis_dla_gst_lgl_1 AS 11a\n"
                + "        LEFT JOIN mis_gst_commcode AS cc ON 11a.COMM_CODE = cc.COMM_CODE\n"
                + "        LEFT JOIN mis_gst_zonecode AS zc ON cc.ZONE_CODE = zc.ZONE_CODE\n"
                + "        WHERE 11a.MM_YYYY = '" + month_date + "'  AND 11a.FORUM_CODE IN (6, 7)\n"
                + "        GROUP BY zc.ZONE_NAME, zc.ZONE_CODE, cc.COMM_NAME, cc.COMM_CODE\n"
                + "    ),\n"
                + "    \n"
                + "    cte_11_prev AS (\n"
                + "        SELECT zc.ZONE_NAME, zc.ZONE_CODE, cc.COMM_NAME, cc.COMM_CODE,\n"
                + "               SUM(CASE WHEN 11a.FORUM_CODE = 6 THEN DEPARTMENT_CLOSING_NO + TAXPAYER_CLOSING_NO ELSE 0 END) AS col4a,\n"
                + "               SUM(CASE WHEN 11a.FORUM_CODE = 7 THEN DEPARTMENT_CLOSING_NO + TAXPAYER_CLOSING_NO ELSE 0 END) AS col4c\n"
                + "        FROM mis_dla_gst_lgl_1 AS 11a\n"
                + "        LEFT JOIN mis_gst_commcode AS cc ON 11a.COMM_CODE = cc.COMM_CODE\n"
                + "        LEFT JOIN mis_gst_zonecode AS zc ON cc.ZONE_CODE = zc.ZONE_CODE\n"
                + "        WHERE 11a.MM_YYYY = '" + prev_month_new + "' AND 11a.FORUM_CODE IN (6, 7)\n"
                + "        GROUP BY zc.ZONE_NAME, zc.ZONE_CODE, cc.COMM_NAME, cc.COMM_CODE\n"
                + "    ),\n"
                + "\n"
                + "    -- CTE for Median Calculation (for 11a and 11c)\n"
                + "    median_cte_11a AS (\n"
                + "        SELECT \n"
                + "            CASE WHEN COUNT(numerator11a) % 2 = 0 THEN \n"
                + "                AVG(numerator11a) \n"
                + "            ELSE \n"
                + "                MAX(numerator11a) \n"
                + "            END AS median_11a\n"
                + "        FROM (\n"
                + "            SELECT numerator11a,\n"
                + "                   ROW_NUMBER() OVER (ORDER BY numerator11a) AS rn,\n"
                + "                   COUNT(*) OVER () AS cnt\n"
                + "            FROM cte_11\n"
                + "        ) AS temp\n"
                + "        WHERE rn IN (FLOOR((cnt + 1) / 2), CEIL((cnt + 1) / 2))\n"
                ...
                + "    median_cte_11c AS (\n"
                + "        SELECT \n"
                + "            CASE WHEN COUNT(numerator11c) % 2 = 0 THEN \n"
                + "                AVG(numerator11c) \n"
                + "            ELSE \n"
                + "                MAX(numerator11c) \n"
                + "            END AS median_11c\n"
                + "        FROM (\n"
                + "            SELECT numerator11c,\n"
                + "                   ROW_NUMBER() OVER (ORDER BY numerator11c) AS rn,\n"
                + "                   COUNT(*) OVER () AS cnt\n"
                + "            FROM cte_11\n"
                + "        ) AS temp\n"
                + "        WHERE rn IN (FLOOR((cnt + 1) / 2), CEIL((cnt + 1) / 2))\n"
                + "    ),\n"
                + "\n"
                + "    -- CTE for Query 2 and Query 4 (FORUM_CODE = 6 and 7 for MM_YYYY = '" + month_date + "' )\n"
                + "    cte_11_ab AS (\n"
                + "        SELECT zc.ZONE_NAME, zc.ZONE_CODE, cc.COMM_NAME, cc.COMM_CODE,\n"
                + "               SUM(CASE WHEN 11a.FORUM_CODE = 6 THEN DEPARTMENT_CLOSING_NO + TAXPAYER_CLOSING_NO ELSE 0 END) AS col10A,\n"
                + "               SUM(CASE WHEN 11a.FORUM_CODE = 6 THEN DEPARTMENT_BREAKUP_LESS_1YEAR_NO + TAXPAYER_BREAKUP_LESS_1YEAR_NO ELSE 0 END) AS col12A,\n"
                + "               SUM(CASE WHEN 11a.FORUM_CODE = 7 THEN DEPARTMENT_CLOSING_NO + TAXPAYER_CLOSING_NO ELSE 0 END) AS col10B,\n"
                + "               SUM(CASE WHEN 11a.FORUM_CODE = 7 THEN DEPARTMENT_BREAKUP_LESS_1YEAR_NO + TAXPAYER_BREAKUP_LESS_1YEAR_NO ELSE 0 END) AS col12B\n"
                + "        FROM mis_dla_gst_lgl_1a AS 11a\n"
                + "        LEFT JOIN mis_gst_commcode AS cc ON 11a.COMM_CODE = cc.COMM_CODE\n"
                + "        LEFT JOIN mis_gst_zonecode AS zc ON cc.ZONE_CODE = zc.ZONE_CODE\n"
                + "        WHERE 11a.MM_YYYY = '" + month_date + "'  AND 11a.FORUM_CODE IN (6, 7)\n"
                ...
                + "    cte_11_ab_prev AS (\n"
                + "        SELECT zc.ZONE_NAME, zc.ZONE_CODE, cc.COMM_NAME, cc.COMM_CODE,\n"
                + "               SUM(CASE WHEN 11b.FORUM_CODE = 6 THEN DEPARTMENT_CLOSING_NO + TAXPAYER_CLOSING_NO ELSE 0 END) AS col10A_prev,\n"
                + "               SUM(CASE WHEN 11b.FORUM_CODE = 6 THEN DEPARTMENT_BREAKUP_LESS_1YEAR_NO + TAXPAYER_BREAKUP_LESS_1YEAR_NO ELSE 0 END) AS col12A_prev,\n"
                + "               SUM(CASE WHEN 11b.FORUM_CODE = 7 THEN DEPARTMENT_CLOSING_NO + TAXPAYER_CLOSING_NO ELSE 0 END) AS col10B_prev,\n"
                + "               SUM(CASE WHEN 11b.FORUM_CODE = 7 THEN DEPARTMENT_BREAKUP_LESS_1YEAR_NO + TAXPAYER_BREAKUP_LESS_1YEAR_NO ELSE 0 END) AS col12B_prev\n"
                + "        FROM mis_dla_gst_lgl_1b AS 11b\n"
                + "        LEFT JOIN mis_gst_commcode AS cc ON 11b.COMM_CODE = cc.COMM_CODE\n"
                + "        LEFT JOIN mis_gst_zonecode AS zc ON cc.ZONE_CODE = zc.ZONE_CODE\n"
                + "        WHERE 11b.MM_YYYY ='" + prev_month_new + "' AND 11b.FORUM_CODE IN (6, 7)\n"
                + "        GROUP BY zc.ZONE_NAME, zc.ZONE_CODE, cc.COMM_NAME, cc.COMM_CODE\n"
                + "    )\n"
                + "\n"
                + "-- Final SELECT merging all calculations:\n"
                + "SELECT \n"
                + "    cte_11.ZONE_NAME, cte_11.ZONE_CODE, cte_11.COMM_NAME,\n"
                + "    cte_11.numerator11a, \n"
                + "    (cte_11.numerator11a / cte_11_prev.col4a) AS total_score_11a,\n"
                + "    median_cte_11a.median_11a,\n"
                + "    \n"
                + "    (((cte_11_ab.col10A - cte_11_ab.col12A) + (cte_11_ab_prev.col10A_prev - cte_11_ab_prev.col12A_prev)) / \n"
                + "    (cte_11_ab.col10A + cte_11_ab_prev.col10A_prev)) AS total_score_11b,\n"
                + "    \n"
                + "    cte_11.numerator11c, \n"
                + "    (cte_11.numerator11c / NULLIF(cte_11_prev.col4c, 0)) AS total_score_11c,\n"
                + "    median_cte_11c.median_11c,\n"
                + "    \n"
                + "    (((cte_11_ab.col10B - cte_11_ab.col12B) + (cte_11_ab_prev.col10B_prev - cte_11_ab_prev.col12B_prev)) / \n"
                + "    (cte_11_ab.col10B + cte_11_ab_prev.col10B_prev)) AS total_score_11d\n"
                + "FROM \n"
                + "    cte_11\n"
                + "    INNER JOIN cte_11_prev ON cte_11.COMM_CODE = cte_11_prev.COMM_CODE\n"
                + "    CROSS JOIN median_cte_11a\n"
                + "    CROSS JOIN median_cte_11c\n"
                + "    INNER JOIN cte_11_ab ON cte_11.COMM_CODE = cte_11_ab.COMM_CODE\n"
                + "    INNER JOIN cte_11_ab_prev ON cte_11_ab.COMM_CODE = cte_11_ab_prev.COMM_CODE\n"
                + "WHERE \n"
                + "    cte_11.ZONE_CODE = 52;\n"
                + "";
        return query_assessment;
    ...
    public String QueryFor_Appeals_11_ParticularSubparameterWise(String month_date, String zone_code){
        //              '" + month_date + "'	 '" + prev_month_new + "'	'" + zone_code + "'		'" + come_name + "' 	'" + next_month_new + "'
        String prev_month_new = DateCalculate.getPreviousMonth(month_date);
        String query_assessment = "WITH cte_A AS (\n" +
                "    SELECT zc.ZONE_NAME, zc.ZONE_CODE, SUM(DEPARTMENT_DISPOSAL_NO + TAXPAYER_DISPOSAL_NO) AS numerator\n" +
                "    FROM mis_dla_gst_lgl_1 AS 11a\n" +
                "    LEFT JOIN mis_gst_commcode cc ON 11a.COMM_CODE = cc.COMM_CODE\n" +
                "    LEFT JOIN mis_gst_zonecode zc ON cc.ZONE_CODE = zc.ZONE_CODE\n" +
                "    WHERE 11a.MM_YYYY = '" + month_date + "' AND FORUM_CODE = 6 GROUP BY zc.ZONE_CODE, zc.ZONE_NAME\n" +
                "), \n" +
                "cte1_A AS (\n" +
                "    SELECT zc.ZONE_NAME, zc.ZONE_CODE,SUM(DEPARTMENT_CLOSING_NO + TAXPAYER_CLOSING_NO) AS col4\n" +
                "    FROM mis_dla_gst_lgl_1 AS 11a\n" +
                "    LEFT JOIN mis_gst_commcode cc ON 11a.COMM_CODE = cc.COMM_CODE\n" +
                "    LEFT JOIN mis_gst_zonecode zc ON cc.ZONE_CODE = zc.ZONE_CODE\n" +
                "    WHERE 11a.MM_YYYY = '" + prev_month_new + "' AND FORUM_CODE = 6 GROUP BY zc.ZONE_CODE, zc.ZONE_NAME\n" +
                "), \n" +
                "median_cte_A AS (\n" +
                "    SELECT AVG(numerator) AS median\n" +
                "    FROM (SELECT numerator, \n" +
                "                 ROW_NUMBER() OVER (ORDER BY numerator) AS rn,\n" +
                "                 COUNT(*) OVER () AS cnt\n" +
                "          FROM cte_A\n" +
                "         ) AS temp\n" +
                "    WHERE rn IN (FLOOR((cnt + 1) / 2), CEIL((cnt + 1) / 2))\n" +
                ...
                "),\n" +
                "cte_C AS (\n" +
                "    SELECT zc.ZONE_NAME, zc.ZONE_CODE, SUM(11a.DEPARTMENT_DISPOSAL_NO + 11a.TAXPAYER_DISPOSAL_NO) AS numerator\n" +
                "    FROM mis_dla_gst_lgl_1 AS 11a\n" +
                "    LEFT JOIN mis_gst_commcode cc ON 11a.COMM_CODE = cc.COMM_CODE\n" +
                "    LEFT JOIN mis_gst_zonecode zc ON cc.ZONE_CODE = zc.ZONE_CODE\n" +
                "    WHERE 11a.MM_YYYY = '" + month_date + "' AND 11a.FORUM_CODE = 7 GROUP BY zc.ZONE_NAME, zc.ZONE_CODE\n" +
                ...
                "    WHERE 11b.MM_YYYY = '" + month_date + "' AND FORUM_CODE = 7 GROUP BY zc.ZONE_CODE\n" +
                ")\n" +
                "SELECT cte_A.ZONE_NAME, cte_A.ZONE_CODE, cte_A.numerator, (cte_A.numerator / cte1_A.col4) AS total_score, concat(cte_A.numerator ,'/', cte1_A.col4) as absvl,\n" +
                "       median_cte_A.median, 'GST11A' AS gst\n" +
                "FROM cte_A\n" +
                "INNER JOIN cte1_A ON cte_A.ZONE_CODE = cte1_A.ZONE_CODE\n" +
                "CROSS JOIN median_cte_A\n" +
                "WHERE cte_A.ZONE_CODE = '" + zone_code + "'\n" +
                "\n" +
                "UNION ALL\n" +
                "\n" +
                "SELECT cte_B.ZONE_NAME, cte_B.ZONE_CODE, 0 AS numerator, \n" +
                "       CASE WHEN (cte_B.col10A + cte1_B.col10B) = 0 THEN NULL \n" +
                "            ELSE (((cte_B.col10A - cte_B.col12A) + (cte1_B.col10B - cte1_B.col12B)) / (cte_B.col10A + cte1_B.col10B)) \n" +
                "       END AS total_score, concat(((cte_B.col10A - cte_B.col12A) + (cte1_B.col10B - cte1_B.col12B)) ,'/', (cte_B.col10A + cte1_B.col10B)) as absvl,\n" +
                "       0 AS median, 'GST11B' AS gst\n" +
                "FROM cte_B\n" +
                "INNER JOIN cte1_B ON cte_B.ZONE_CODE = cte1_B.ZONE_CODE\n" +
                "WHERE cte_B.ZONE_CODE = '" + zone_code + "'\n" +
                "\n" +
                "UNION ALL\n" +
                "\n" +
                "SELECT cte_C.ZONE_NAME, cte_C.ZONE_CODE, cte_C.numerator, (cte_C.numerator / NULLIF(cte1_C.col4, 0)) AS total_score,\n" +
                "       concat(cte_C.numerator ,'/', NULLIF(cte1_C.col4, 0)) as absvl, median_cte_C.median, 'GST11C' AS gst\n" +
                "FROM cte_C\n" +
                "INNER JOIN cte1_C ON cte_C.ZONE_CODE = cte1_C.ZONE_CODE\n" +
                "CROSS JOIN median_cte_C\n" +
                "WHERE cte_C.ZONE_CODE = '" + zone_code + "'\n" +
                "\n" +
                "UNION ALL\n" +
                "\n" +
                "SELECT cte_D.ZONE_NAME, cte_D.ZONE_CODE, 0 AS numerator, \n" +
                "       (((cte_D.col10A - cte_D.col12A) + (cte1_D.col10B - cte1_D.col12B)) / (cte_D.col10A + cte1_D.col10B)) AS total_score, \n" +
                "        concat(((cte_D.col10A - cte_D.col12A) + (cte1_D.col10B - cte1_D.col12B)) ,'/', \n" +
                "        (cte_D.col10A + cte1_D.col10B)) as absvl, 0 AS median, 'GST11D' AS gst\n" +
                "FROM cte_D\n" +
                "INNER JOIN cte1_D ON cte_D.ZONE_CODE = cte1_D.ZONE_CODE\n" +
                "WHERE cte_D.ZONE_CODE = '" + zone_code + "';\n";
        return query_assessment;
    }
    //  for all commissary wise || for 4no url
