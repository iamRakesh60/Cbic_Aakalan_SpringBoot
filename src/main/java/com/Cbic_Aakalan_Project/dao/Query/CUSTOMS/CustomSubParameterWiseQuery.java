...
public class CustomSubParameterWiseQuery {
    ...
    public String QueryFor_cus6a_CommissonaryWise(String month_date, String zone_code){
        ...
        String queryCustom5b="WITH data AS (\n"
                ...
                + "    INNER JOIN mis_gst_zonecode AS zc ON zc.ZONE_CODE = cc.ZONE_CODE \n"
                + "    INNER JOIN mis_dri_cus_3b AS 15c ON 14c.COMM_CODE = 15c.COMM_CODE AND 15c.MM_YYYY = ? \n" //--month_date
                + "    INNER JOIN mis_dri_cus_3a AS 14c_prev ON 14c.COMM_CODE = 14c_prev.COMM_CODE AND 14c_prev.MM_YYYY = ? \n" //--prev_month_new
                + "    INNER JOIN mis_dri_cus_3b AS 15c_prev ON 14c.COMM_CODE = 15c_prev.COMM_CODE AND 15c_prev.MM_YYYY = ?\n" //--prev_month_new
                + "),\n"
                + "ranked_data AS (\n"
                + "    SELECT \n"
                + "        d.*,\n"
                ...
                + "WHERE rd.ZONE_CODE =  ? -- Apply the filter here\n"
    ...
    public String QueryFor_cus7a_ZoneWise(String month_date){
        ...
        String queryCustom7a="WITH calculated_data AS (\n" +
                "    SELECT zc.ZONE_NAME, cc.ZONE_CODE, \n" +
                "        IFNULL(SUM(CASE WHEN c14.MM_YYYY = ? THEN c14.NO_PROSECUTION_NOT_LAUNCH ELSE 0 END), 0) AS col8, \n" +
                "        IFNULL(SUM(CASE WHEN c14.MM_YYYY = ? THEN c14.NO_PROSECUTION_HAS_SANCTIONED ELSE 0 END), 0) AS col5\n" +
                "    FROM mis_gst_commcode AS cc\n" +
    ...
    public String QueryFor_cus11a_ZoneWise(String month_date){
        ...
        String queryCustom10a="WITH calculated_data AS (\n" +
                "    SELECT zc.ZONE_NAME, cc.ZONE_CODE, \n" +
                "        IFNULL(SUM(CASE WHEN c14.MM_YYYY = ? THEN c14.CLOSING_BALANCE_NO ELSE 0 END), 0) AS col9, \n" +
                "        IFNULL(SUM(CASE WHEN c14.MM_YYYY = DATE_FORMAT(DATE_SUB(?, INTERVAL 1 MONTH), '%Y-%m-%d') THEN c14.CLOSING_BALANCE_NO ELSE 0 END), 0) AS col1, \n" +
                "        IFNULL(SUM(CASE WHEN c14.MM_YYYY = ? THEN c14.RECEIVED_EXPIRED_WH_NO ELSE 0 END), 0) AS col3 \n" +
                "    FROM mis_gst_commcode AS cc \n" +
    ...
    public String QueryFor_cus11a_CommissonaryWise(String month_date, String zone_code){
        ...
        String queryCustom10a="WITH calculated_data AS (\n" +
                ...
                "    INNER JOIN mis_gst_zonecode AS zc ON zc.ZONE_CODE = cc.ZONE_CODE \n" +
                "    WHERE c14.MM_YYYY IN (?, DATE_FORMAT(DATE_SUB(?, INTERVAL 1 MONTH), '%Y-%m-%d')) \n" +
                "        AND cc.ZONE_CODE NOT IN ('70', '59', '18', '53', '63', '60', '65') AND cc.ZONE_CODE = ? \n" +
                "    GROUP BY zc.ZONE_CODE, zc.ZONE_NAME, cc.ZONE_CODE, cc.COMM_NAME\n" +
                "), \n" +
                "ranked_data AS (\n" +
                "    SELECT cd.*, \n" +
    ...
    public String QueryFor_cus12a_CommissonaryWise(String month_date, String zone_code){
        ...
        String queryCustom12a ="WITH cte_1 AS (\n" +
                ...
                "    RIGHT JOIN mis_gst_commcode AS cc ON 14c.COMM_CODE = cc.COMM_CODE \n" +
                "    LEFT JOIN mis_gst_zonecode AS zc ON zc.ZONE_CODE = cc.ZONE_CODE \n" +
                "    WHERE 14c.MM_YYYY = ? AND 14c.FORUM_CODE = 5\n" +
                "),\n" +
                "cte_2 AS (\n" +
